...
#define TEST_SIZES 3
#define SIZES {10, 16, 20}
#define TEST_THREADS 4
#define THREADS {1, 2, 4, 8}
#define ITERS 1
#define inf 1e9
...
int verify_tsp(uint n, uint ** w, uint ** dp, uint * tour){
    ...
    long long min_weight = tsp_basic(n, w, dp);
    long long t_weight = 0;
    int prev = n;
    for (int i = 0; i < n; i++){
...
uint tsp(uint n, uint ** w, uint ** dp, uint ** prev, uint * tour, uint num_threads){
    ...
    int mask_size = (1 << (n-1));
    for (uint i = 1; i < mask_size; i++){
        #pragma omp parallel for num_threads (num_threads) private(temp, k)
        for (uint j = 0; j < n-1; j++){
            if (i & (1<<j)){ // j in the set
                continue;
            }
